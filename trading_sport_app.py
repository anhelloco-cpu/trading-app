                    ...
                    with st.expander(f"{icono_est} {op['partido']} | Ref: {op['codigo']} | Estado: {op['estado']}"):
                        ...
                        if op['estado'] == "EN VIVO":
                            ...
                            if accion_esports == "📸 Evaluar Asedio y Cazar Cuota":
                                    ...
                                    if st.button("🔄 Sincronizar Info", key=f"btn_sync_seg_{op['codigo']}"):
                                        try:
                                            ...
                                            if res_sync.data:
                                                ...
                                                # CARGA EN SESSION STATE CON LAS MISMAS LLAVES DE LOS INPUTS
                                ...
                                with c_top1:
                                    min_ui_val = st.session_state.get(f"min_es_{op['codigo']}", max(0, min(95, int(minuto_sugerido))))
                                    minuto_actual = st.number_input("⏱️ Minuto:", min_value=0, max_value=120, value=min_ui_val, step=1, key=f"min_es_{op['codigo']}")
                                with c_top2:
                                    val_cuota_obj = float(op.get('cuota_objetivo') or 1.01)
                                    if val_cuota_obj < 1.01: val_cuota_obj = 1.01
                                    c_ui_val = st.session_state.get(f"c_live_es_{op['codigo']}", val_cuota_obj)
                                    cuota_input_es = st.number_input("📉 Cuota Cobertura:", min_value=1.01, step=0.01, value=c_ui_val, key=f"c_live_es_{op['codigo']}")
                                ...
                                with col_t1:
                                    g_l_val = st.session_state.get(f"g_l_es_{op['codigo']}", int(ultima_foto.get('goles_local', 0)))
                                    g_local = st.number_input(f"⚽ Goles de {eq_local}", min_value=0, value=g_l_val, key=f"g_l_es_{op['codigo']}")
                                    atqt_l_val = st.session_state.get(f"atqt_l_es_{op['codigo']}", int(ultima_foto.get('atqt_local', 80)))
                                    atqt_local = st.number_input(f"📊 Atq. Tot. de {eq_local}", min_value=0, value=atqt_l_val, key=f"atqt_l_es_{op['codigo']}")
                                    atk_l_val = st.session_state.get(f"atk_l_es_{op['codigo']}", int(ultima_foto.get('atkp_local', 0)))
                                    atkp_local = st.number_input(f"🔥 Peligro {eq_local}", min_value=0, value=atk_l_val, key=f"atk_l_es_{op['codigo']}")
                                
                                with col_t2:
                                    g_v_val = st.session_state.get(f"g_v_es_{op['codigo']}", int(ultima_foto.get('goles_vis', 0)))
                                    g_vis = st.number_input(f"⚽ Goles de {eq_vis}", min_value=0, value=g_v_val, key=f"g_v_es_{op['codigo']}")
                                    atqt_v_val = st.session_state.get(f"atqt_v_es_{op['codigo']}", int(ultima_foto.get('atqt_vis', 50)))
                                    atqt_vis = st.number_input(f"📊 Atq. Tot. de {eq_vis}", min_value=0, value=atqt_v_val, key=f"atqt_v_es_{op['codigo']}")
                                    atk_v_val = st.session_state.get(f"atk_v_es_{op['codigo']}", int(ultima_foto.get('atkp_vis', 0)))
                                    atkp_vis = st.number_input(f"🔥 Peligro {eq_vis}", min_value=0, value=atk_v_val, key=f"atk_v_es_{op['codigo']}")
                                ...
                                ird = min(100.0, ((atkp_local + atkp_vis) / max(1, minuto_actual)) * 45.0) if minuto_actual > 0 else 0.0
                                    ...
                                    with col_btn2:
                                        if st.button("🛡️ FREEBET (Opción B)", key=f"btn_cob_b_{op['codigo']}", use_container_width=True):
                                            ...
                                            supabase.table("historial_trading").update({
                                                ...
                                                "cuota_cazada_real": float(cuota_salida),
                                                "hora_cobertura": hora_actual,
                                                "plataforma_cobertura": f"{plataforma_cob} [FREEBET]",
                                ...
                                with st.form(f"get_dir_es_{op['codigo']}"):
                                    ...
                                    if st.form_submit_button("Registrar Liquidación Directa"):
                                        ...
                                        if "Ganó" in resultado_directo: texto_cierre = f"Cierre Directo {etiqueta_db}: Ganó Inicial"
                                        else: texto_cierre = f"Cierre Directo {etiqueta_db}: Perdió Inicial"
                            ...
                            if es_apuesta_libre:
                                ...
                                if accion == "Evaluar Asedio y Cobertura (IRD)":
                                    ...
                                    if res_fotos.data:
                                        ...
                                        ultima_foto = {'goles_local': 0, 'goles_vis': 0, 'atkp_local': 0, 'atkp_vis': 0, 'atqt_local': 80, 'atqt_vis': 50}
                                        min_base = 0
                                    ...
                                    minuto_actual = st.number_input("⏱️ Minuto del Partido:", min_value=0, max_value=110, value=st.session_state.get(f"min_{op['codigo']}", minuto_sugerido), step=1, key=f"min_in_{op['codigo']}")
                                    ...
                                    with col_t1:
                                        ...
                                        g_local = st.number_input(f"⚽ Goles", min_value=0, value=st.session_state.get(f"g_l_{op['codigo']}", int(ultima_foto.get('goles_local', 0))), key=f"g_l_in_{op['codigo']}")
                                        atqt_local = st.number_input(f"📊 Atq Totales", min_value=0, value=st.session_state.get(f"atqt_l_{op['codigo']}", int(ultima_foto.get('atqt_local', 80))), key=f"atqt_l_in_{op['codigo']}")
                                        atkp_local = st.number_input(f"🔥 Atq. Peligrosos", min_value=0, value=st.session_state.get(f"atk_l_{op['codigo']}", int(ultima_foto.get('atkp_local', 0))), key=f"atk_l_in_{op['codigo']}")
                                    ...
                                    with col_t2:
                                        ...
                                        g_vis = st.number_input(f"⚽ Goles", min_value=0, value=st.session_state.get(f"g_v_{op['codigo']}", int(ultima_foto.get('goles_vis', 0))), key=f"g_v_in_{op['codigo']}")
                                        atqt_vis = st.number_input(f"📊 Atq Totales", min_value=0, value=st.session_state.get(f"atqt_v_{op['codigo']}", int(ultima_foto.get('atqt_vis', 50))), key=f"atqt_v_in_{op['codigo']}")
                                        atkp_vis = st.number_input(f"🔥 Atq. Peligrosos", min_value=0, value=st.session_state.get(f"atk_v_{op['codigo']}", int(ultima_foto.get('atkp_vis', 0))), key=f"atk_v_in_{op['codigo']}")
                                    ...
                                    # ====================================================================
                                    # ⚡ MOTOR DE MOMENTUM (EXTRAE Y GUARDA AL INSTANTE PARA ESTRATEGIA 2)
                                    # ====================================================================
                                    if st.button("📸 Guardar Foto de Auditoría", key=f"btn_foto_{op['codigo']}", use_container_width=True, type="primary"):
                                        try:
                                            nueva_foto = {
                                                "codigo_posicion": str(op['codigo']),
                                                "minuto_evaluado": int(minuto_actual),
                                                ...
                                                "atqt_local": int(atqt_local or 80),
                                                "atqt_vis": int(atqt_vis or 50),
